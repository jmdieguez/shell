...
expand_environ_var(char *arg)
{
	// Your code here
	if (arg[0] == '$') {
		if (!strcmp(arg, "$?")) {  // Magic var '?' prints last process exit status
		...
		if (!var || !strlen(var))  // If empty var
			return NULL;

		if (strlen(var) >
		    strlen(arg)) {  // Big var value, needs reallocation
			char *new_ptr;
			new_ptr = realloc(arg, strlen(var) + 1);

			if (!new_ptr) {
				fprintf_debug(
				        stderr, "Error while reallocating environment variable value\n");
				return NULL;
			}
